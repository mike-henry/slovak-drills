...
class MockNoun {
  declinate(caseType: CASE_TYPE, plural: boolean) {
    return new DerivedWord('nounDerived', 'nounExpl', ['noun-section1', 'noun-section2']);
  }
}

class MockAdjective {
  declinate(caseType: CASE_TYPE, noun: MockNoun, plural: boolean) {
    return new DerivedWord('adjDerived', 'adjExpl', ['adjective-section1', 'adjective-section2']);
  }
}
...
describe('declinateAdjectiveWithNoun (mocked)', () => {
  test('combines adjective and noun declinations into a DerivedWord', () => {
    ...
    const result = declinateAdjectiveWithNoun(adjective as any, noun as any, 'GENITIVE' as CASE_TYPE, true);
    ...
    expect(result.explanation).toBe('for adjective: adjExpl, for noun: nounExpl');
