...
import { WORD, CASE_TYPE, Gender, Person } from "../WordTypes";
...
export default class Verb extends WORD {
  reflexive: boolean;
  presentStem?: string;
  presentMap?: Partial<Record<Person, string>>;
  caseType: CASE_TYPE;
  constructor(
    sk: string,
    en: string,
  ...
  conjugatePresent(person: Person, gender?: Gender): DerivedWord {
    const presentConjugator = this.getConjugator(person);
    return presentConjugator.conjugate(person, gender);
  }

  private getConjugator(person: Person): PresentConjugator {
