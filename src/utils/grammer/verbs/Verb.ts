...
export default class Verb extends WORD {
  ...
  static getLabels(): string[] {
    return Verb.allLabels.values().toArray().sort();
  }

  static getRandom = (): Verb => {
    while (!LOADED) {
      /* wait */
    } // no better way for now
    const filterFn: (item: Verb) => boolean =
      filter.length == 0 ? (n) => true : (item: Verb) => filter.some((label) => item.labels.includes(label));
    return WORD.getRandomWord<Verb>(Verb.verbs, filterFn);
  };
  ...
  private getConjugator(person: Pronoun): PresentConjugator {
    ...
    switch (this.getVerbClass()) {
      case 'at':
        return new AtConjugator(this);
      case 'it-long':
        return new ItConjugator(this);
      case 'it-short':
        return new ItShortConjugator(this);
      case 'iet':
        return new IetConjugator(this);
      case 'ovat':
        return new OvatConjugator(this);
      case 'nut':
        return new NutConjugator(this);
      case 'st':
        return new StConjugator(this);
      case 'irregular':
        return new IrregularConjugator(this);
      default:
        return new IrregularConjugator(this);
    }
  }
...
let filter: string[] = [];
let LOADED = false;
bus.on('verb-active-labels-updated', (labels) => {
  filter = labels;
});
loadWords(VERBS, Verb.verbs, DEFAULT_VERBS, Verb.fromRaw).then((result) => {
  LOADED = true;
  Verb.verbs = result;
  bus.emit('verb-all-labels-updated', Verb.getLabels());
  console.info(`✅ verbs loaded: ${Verb.verbs.length} from ${VERBS}`);
});
