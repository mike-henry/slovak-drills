...
const buildNextItem: () => Item = () => ({ noun: getRandomNoun(), isPlural: randomBoolean() ,adjective:getRandomAdjective()})

const  sk= (item:Item) => `${item.adjective.sk} ${item.noun.sk}`
const  en= (item:Item) => `${item.adjective.en} ${item.noun.en}`

const expected = (item:Item) => declinateAdjectiveWithNoun(item.adjective, item.noun, caseName as CASE_TYPE, item.isPlural)
