...
function deriveDocumentation(caseType: CASE_TYPE, form: 'short' | 'long' | 'afterPrep'): string[] {
  const baseUri = `pronoun://${caseType}`;
  let docs: string[] = [];

  switch (form) {
    case 'short':
      docs.push(`${baseUri}?pronouns-introduction&pronouns-short-forms`);
      break;
    case 'long':
      docs.push(`${baseUri}?pronouns-introduction&pronouns-long-forms`);
      break;
    case 'afterPrep':
      docs.push(`${baseUri}?pronouns-introduction&pronouns-after-prep`);
      break;
  }
...
export function getPronounForm(
  ...
  return new DerivedWord(derived, explanation, deriveDocumentation(caseType, form));
}
