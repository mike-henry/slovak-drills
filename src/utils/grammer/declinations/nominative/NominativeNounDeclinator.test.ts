...
describe('Feminine nouns — singular and plural', () => {
  ...
  const nohavice: Noun = Noun.fromRaw({
    sk: 'nohavice',
...
describe('Additional B2 nouns — plural tests', () => {
  const dieta = Noun.fromRaw({ sk: 'dieťa', gender: Gender.Neutral, animate: false, en: 'child' });
  const mladza = Noun.fromRaw({ sk: 'mláďa', gender: Gender.Neutral, animate: false, en: 'young animal' });
  const kon = Noun.fromRaw({ sk: 'kôň', gender: Gender.Masculine, animate: false, en: 'horse' });
  const srdce = Noun.fromRaw({ sk: 'srdce', gender: Gender.Neutral, animate: false, en: 'heart' });
  const vysvedcenie = Noun.fromRaw({ sk: 'vysvedčenie', gender: Gender.Neutral, animate: false, en: 'report' });

  const muz = Noun.fromRaw({ sk: 'muž', gender: Gender.Masculine, animate: true, en: 'man' });
  const pan = Noun.fromRaw({ sk: 'pán', gender: Gender.Masculine, animate: true, en: 'sir' });
  const chlapec = Noun.fromRaw({ sk: 'chlapec', gender: Gender.Masculine, animate: true, en: 'boy' });

  const kost = Noun.fromRaw({ sk: 'kosť', gender: Gender.Femenine, animate: false, en: 'bone' });
  const radosť = Noun.fromRaw({ sk: 'radosť', gender: Gender.Femenine, animate: false, en: 'joy' });
  const ulica = Noun.fromRaw({ sk: 'ulica', gender: Gender.Femenine, animate: false, en: 'street' });
  const chemia = Noun.fromRaw({ sk: 'chémia', gender: Gender.Femenine, animate: false, en: 'chemistry' });

  test('Neuter irregular plurals', () => {
    //  expect(dieta.declinate(CASE, true).derived).toBe('deti');
    //    expect(mladza.declinate(CASE, true).derived).toBe('mláďatá');
    // expect(kon.declinate(CASE, true).derived).toBe('kone');
    expect(srdce.declinate(CASE, true).derived).toBe('srdcia');
    expect(vysvedcenie.declinate(CASE, true).derived).toBe('vysvedčenia');
  });

  test('Masculine animate plurals', () => {
    expect(muz.declinate(CASE, true).derived).toBe('muži');
    expect(pan.declinate(CASE, true).derived).toBe('páni');
    expect(chlapec.declinate(CASE, true).derived).toBe('chlapci');
  });

  test('Feminine consonant and -a ending plurals', () => {
    expect(kost.declinate(CASE, true).derived).toBe('kosti');
    expect(radosť.declinate(CASE, true).derived).toBe('radosti');
    expect(ulica.declinate(CASE, true).derived).toBe('ulice');
    expect(chemia.declinate(CASE, true).derived).toBe('chémie');
  });
});
